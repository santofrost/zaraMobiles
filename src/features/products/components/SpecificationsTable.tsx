...
export default function SpecificationsTable({
  ...
  return (
    <section className="mt-8 border-t border-gray-100 px-4 py-12 sm:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <dl className="border-t border-gray-200">
          {rows.map((row) => {
            const translationTarget = `specs.${row.key}` as keyof typeof t extends never
              ? never
              : TranslationKey;
            ...
            return (
              ...
              >
                <dt className="w-48 shrink-0 text-xs font-medium tracking-wider text-gray-900 uppercase">
                  {t(translationTarget) !== translationTarget ? t(translationTarget) : row.key}
                </dt>
                <dd className="m-0 text-sm text-gray-600">{row.value}</dd>
              </div>
            );
          })}
        </dl>
